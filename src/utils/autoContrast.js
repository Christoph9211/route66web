...
export function wcagContrastColor(bg) {
    ...
    // Pick whichever passes AA and has higher ratio
    if (whiteContrast >= 4.5 && whiteContrast > blackContrast) return "#fff";
    if (blackContrast >= 4.5 && blackContrast >= whiteContrast) return "#111";

    // If both fail, adjust the chosen color's brightness until it passes
    let [cr, cg, cb] = whiteContrast > blackContrast ? [255, 255, 255] : [17, 17, 17];
    let step = whiteContrast > blackContrast ? -1 : 1;
    while (contrast(luminance, 0.2126 * L(cr) + 0.7152 * L(cg) + 0.0722 * L(cb)) < 4.5) {
