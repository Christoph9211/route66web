  ...
  if (violations.length) {
    ...
    const payload = violations.flatMap(v =>
      v.nodes.map(n => {
        ...
        const colorData = checks.find(c => c?.id?.includes('color')) as any;
        const data = (colorData && (colorData.data || colorData.relatedNodes?.[0]?.color)) || (colorData?.relatedNodes?.[0] || {});
        ...
        const ratio = (colorData?.data?.contrastRatio) ?? (data?.contrastRatio) ?? null;
        return {
          rule: v.id,
          help: v.help,
          impact: v.impact,
    ...
    console.error(`Axe found ${violations.length} color-contrast violation groups`);
